...
const Crypto = require('./bitcoinnova-crypto')
...
class CryptoNote {
   ...
   * @property {number} [coinUnitPlaces=6] - The number of decimal places in the human readable form of the amount
   * @property {number} [addressPrefix=78] - The decimal representation of the wallet address prefix
   * @property {number} [keccakIterations=1] - The number of KDF iterations to perform for KDF operations
   * @property {number} [defaultNetworkFee=10000] - The default network fee for transactions in atomic units
   ...
   * @param {number} [addressPrefix=78] - decimal representation of the CryptoNote address prefix
  ...
  underivePublicKey (derivation, outputIndex, outputKey) {
    ...
    const [err, key] = BitcoinNovaCrypto.underivePublicKey(derivation, outputIndex, outputKey)
...
function cnFastHash (input) {
  ...
  const [err, hash] = BitcoinNovaCrypto.cn_fast_hash(input)
...
function derivePublicKey (derivation, outputIndex, publicKey) {
  ...
  const [err, key] = BitcoinNovaCrypto.derivePublicKey(derivation, outputIndex, publicKey)
...
function deriveSecretKey (derivation, outputIndex, privateKey) {
  ...
  const [err, key] = BitcoinNovaCrypto.deriveSecretKey(derivation, outputIndex, privateKey)
...
function generateDeterministicSubwalletKeys (privateKey, walletIndex) {
  ...
  const [err, keys] = BitcoinNovaCrypto.generateDeterministicSubwalletKeys(privateKey, walletIndex)
...
function generateKeyDerivation (transactionPublicKey, privateViewKey) {
  ...
  const [err, derivation] = BitcoinNovaCrypto.generateKeyDerivation(transactionPublicKey, privateViewKey)
...
function generateKeyImage (publicKey, privateKey) {
  ...
  const [err, keyImage] = BitcoinNovaCrypto.generateKeyImage(publicKey, privateKey)
...
function checkRingSignatures (transactionPrefixHash, keyImage, inputKeys, signatures) {
  ...
  if (userCryptoFunctions.checkRingSignatures) {
    ...
    return BitcoinNovaCrypto.checkRingSignatures(transactionPrefixHash, keyImage, inputKeys, signatures)
  }
}
...
function generateRingSignaturesPrimitive (transactionPrefixHash, keyImage, inputKeys, privateKey, realIndex) {
  ...
  if (userCryptoFunctions.generateRingSignatures) {
    ...
    const [err, sigs] = BitcoinNovaCrypto.generateRingSignatures(transactionPrefixHash, keyImage, inputKeys, privateKey, realIndex)
...
function generateSignature (digestHash, publicKey, privateKey) {
  ...
  if (userCryptoFunctions.generateSignature) {
    ...
    [err, signature] = BitcoinNovaCrypto.generateSignature(digestHash, publicKey, privateKey)
  }
...
function privateKeyToPublicKey (privateKey) {
  ...
  const [err, key] = BitcoinNovaCrypto.secretKeyToPublicKey(privateKey)
...
function scReduce32 (hex) {
  const [err, result] = BitcoinNovaCrypto.scReduce32(hex)
...
function verifySignature (digestHash, publicKey, signature) {
  ...
  return BitcoinNovaCrypto.checkSignature(digestHash, publicKey, signature)
}
